...
<div id="main">
	<div id="toolbar">
		...
		<shiro:hasPermission name="upms:role:allot"><a class="waves-effect waves-button" href="javascript:;" onclick="allotAction()"><i class="zmdi zmdi-accounts-list"></i> 角色分配</a></shiro:hasPermission>
		<shiro:hasPermission name="upms:role:resource"><a class="waves-effect waves-button" href="javascript:;" onclick="resourceAction()"><i class="zmdi zmdi-key"></i> 角色资源权限</a></shiro:hasPermission>
...
function allotAction() {
    ...
    if (rows.length != 1) {
        ...
        allotRoleId = rows[0].roleId;
        Mcep.Page.Tab.createNewTab("${basePath}/manage/role/allot/"+ allotRoleId, "角色分配");
        /*allotDialog = $.dialog({
            animationSpeed: 300,
            title: '角色分配',
            columnClass: 'large',
            content: 'url:${basePath}/manage/role/allot/' + allotRoleId,
            onContentReady: function () {
                initMaterialInput();
                $('select').select2({
                    placeholder: '',
...
function resourceAction() {
    ...
    if (rows.length != 1) {
        ...
        resourceDialog = $.dialog({
            animationSpeed: 300,
            title: '资源权限',
            content: 'url:${basePath}/manage/role/resources/' + roleId,
            onContentReady: function () {
                initMaterialInput();
                initTree();
            }
