            ...
            <div class="tab-pane fade" id="check" >
                <table id="codeClass" class="timeTable" style="margin-bottom: 20px;margin-top: 20px;">
                    ...
                    <tr >
                        ...
                        <td style="width: 18%;">
                            <input id="catNo" readonly type="text" class="form-control" class="catNoClass" name="catNo" value="${upmsCodeClass.catNo}">
                        </td>
                        <td style="width: 13%;padding-right: 10px;" align="right"><label >分类名称</label></td>
                        <td style="width: 18%;">
                            <input id='catName' readonly class='form-control catName' style='width: 100%' name='catName' type='text' value="${upmsCodeClass.catName}">
...
<script th:inline="javascript">
    ...
    function save(){
        ...
        $.ajax({
            type: 'post',
            url: '/manage/codeClass/update/'+result.catNo,
            //data: result,
    ...
    function initCc() {
        var text = " <tr class='lists' style='width: 100%;height: 30px; '>" +
      ...
      //      "<input id='ison' class='form-control ison' style='width: 100%;padding-top: 0px;padding-bottom: 0px;' name='ison' type='text' >" +
            " <select id='ison' name='ison' class='form-control faultCause' style='width:100%;' >"+
    ...
    $(function(){
        ...
        debugger;
          var cat = $('#catNo').val();
        $.getJSON('/manage/codeClass/code',{catNo: cat}, function(json) {
            for (var i = 0; i < json.length; i ++) {

                var text = " <tr class='lists' style='width: 100%; height: 30px; '>" +
                    "<td style='align-content: center;width: 25%'>" +
                    "<input id='catNo' class='form-control catNo' readonly style=' width: 100%;padding-top: 0px;padding-bottom: 0px;' name='catNo' type='text' value='"+json[i].catNo+"'>" +
                    "</td>" +     //分类编码

                    "<td style='align-content: center;width: 25%'>" +
                    "<input id='cdNo' class='form-control cdNo' style=' width: 100%;padding-top: 0px;padding-bottom: 0px;' name='cdNo' type='text' value='"+json[i].cdNo+"'>" +
                    "</td>" +         //取值编码

                    "<td style='align-content: center;width: 8%'>" +
                    "<input id='cdName' class='form-control cdName' style=' width: 100%;padding-top: 0px;padding-bottom: 0px;' name='cdName' type='text' value='"+json[i].cdName+"'>" +
                    "</td>" +         //取值名称

                    "<td style='align-content: center;width: 8%;'>" +
                 //   "<input id='ison' class='form-control ison' style='width: 100%;padding-top: 0px;padding-bottom: 0px;' name='ison' type='text' value='"+json[i].ison+"'>" +
                    " <select id='ison' name='ison' class='form-control faultCause ison"+i+"' style='width:100%;' >"+
                    "<option  value=''>请选择</option>"+
                    "<option  value='启用'>启用</option>"+
                    "<option  value='禁用'>禁用</option>"+
                    "</select>"+
                    ...
                    "<td style='align-content: center;width: 8%; display: none;'>" +
                    "<input id='old' class='form-control old' style='width: 100%;padding-top: 0px;padding-bottom: 0px;' name='old' type='text' value='1'>" +
                    "</td>" +        //是否旧数据

                    "</tr>";

                $("#code tbody").append(text);
                _state = json[i].ison;

               // $('.ison'+i).select2().val(_state).trigger('change');
                $('.ison'+i).val(_state).trigger('change');
            }
