...
<div id="main">
	<div id="toolbar">
		...
		<shiro:hasPermission name="upms:user:jobs"><a class="waves-effect waves-button" href="javascript:;" onclick="jobsAction()"><i class="zmdi zmdi-accounts"></i> 用户岗位</a></shiro:hasPermission>
		<shiro:hasPermission name="upms:user:resource"><a class="waves-effect waves-button" href="javascript:;" onclick="userAction()"><i class="zmdi zmdi-key"></i> 用户资源权限</a></shiro:hasPermission>
		<shiro:hasPermission name="upms:user:password"><a class="waves-effect waves-button" href="javascript:;" onclick="passwordAction()"><i class="zmdi zmdi-key"></i> 重置密码</a></shiro:hasPermission>
	</div>
...
$(function() {
	...
	$table.bootstrapTable({
		...
		columns: [
			...
			{field: 'userId', title: '编号', sortable: true, align: 'center',sortName:'user_id'},
...
function createAction() {
	createDialog = $.dialog({
		animationSpeed: 300,
		title: '新增用户',
		content: 'url:/manage/user/create',
...
function jobsAction() {
    ...
    if (rows.length != 1) {
        ...
        jobsUserId = rows[0].userId;
        jobsDialog = $.dialog({
            animationSpeed: 300,
            title: '用户岗位',
            content: 'url:${basePath}/manage/user/jobs/' + jobsUserId,
            onContentReady: function () {
                initMaterialInput();
                $('select').select2({
                    placeholder: '请选择用户岗位',
...
function userAction() {
    ...
    if (rows.length != 1) {
        ...
        resourceDialog = $.dialog({
            animationSpeed: 300,
            title: '资源权限',
            content: 'url:${basePath}/manage/user/resources/' + userId,
            onContentReady: function () {
                initMaterialInput();
                initTree();
            }
