...
$(function() {
    ...
    $(document).on('click', '.two-menu>a', function() {
        $(this).next().slideToggle(200);
        $(this).parent().toggleClass('toggled');
    });
	// 个人资料
	...
	$('.switch-systems').click(function () {
		...
		$.cookie('mcep-upms-systemid', systemid);
		$.cookie('mcep-upms-systemname', systemname);
		$.cookie('mcep-upms-systemtitle', systemtitle);
	});
	// 显示cookie菜单
	var systemid = $.cookie('mcep-upms-systemid') || 1;
	var systemname = $.cookie('mcep-upms-systemname') || 'mcep-upms-server';
	var systemtitle = $.cookie('mcep-upms-systemtitle') || '权限管理系统';
	$('.system_menus').hide(0, function () {
		$('.system_' + systemid).show();
	});
	$('body').attr('id', systemname);
...
// 选项卡对象
var Tab = {
	addTab: function(title, url,obj) {
		$(".main-menu li a").removeClass("menu-selected");
		...
		if ($('#tab_' + index).length == 0) {
			// 添加选项卡
			$('.content_tab li').removeClass('cur');
			var tab = '<li id="tab_' + index +'" data-index="' + index + '" class="cur"><a class="waves-effect waves-light">' + title + ' </a> <i class="zmdi zmdi-close"></i> </li>';//<i class="zmdi zmdi-close"></i><
			var $tab = $(tab);
			$('.content_tab>ul').append($tab);
			// 添加iframe
			$('.iframe').removeClass('cur');
			var iframe = '<div id="iframe_' + index + '" class="iframe cur"><iframe id="iframe_' + index + '" class="tab_iframe" src="' + url + '" width="100%" frameborder="0" scrolling="auto" onload="changeFrameHeight(this)"></iframe></div>';
            ...
            $(".zmdi-close",$tab).on("click",function(){
