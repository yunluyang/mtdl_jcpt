...
import net.mcep.upms.client.shiro.session.UpmsSession;
import net.mcep.upms.client.shiro.session.UpmsSessionDao;
...
public class SSOController extends BaseController {
    ...
    private static int TIME_OUT = Integer.valueOf(PropertiesFileUtil.getInstance("mcep-upms-client").get("mcep.upms.session.timeout").toString())/ 1000;
    ...
    public String login(HttpServletRequest request) {
        ...
        // 判断是否已登录，如果已登录，则回跳
        String code = RedisUtil.get(MCEP_UPMS_SERVER_SESSION_ID + "_" + serverSessionId);
        // code校验值
        if (StringUtils.isNotBlank(code)) {
            ...
            if (StringUtils.isBlank(backurl)) {
                ...
                if (backurl.contains("?")) {
                    backurl += "&upms_code=" + code + "&upms_username=" + username+ "&upms_session_id=" + sessionId;
                } else {
                    backurl += "?upms_code=" + code + "&upms_username=" + username+ "&upms_session_id=" + sessionId;
                }
            }
    ...
    public Object login(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) {
        ...
        String hasCode = RedisUtil.get(MCEP_UPMS_SERVER_SESSION_ID + "_" + sessionId);
        // code校验值
        if (StringUtils.isBlank(hasCode)) {
            ...
            RedisUtil.lpush(MCEP_UPMS_SERVER_SESSION_IDS, sessionId.toString());
            // 默认验证帐号密码正确，创建code
            String code = UUID.randomUUID().toString();
            // 全局会话的code
            RedisUtil.set(MCEP_UPMS_SERVER_SESSION_ID + "_" + sessionId, code, (int) subject.getSession().getTimeout() / 1000);
            // code校验值
            RedisUtil.set(MCEP_UPMS_SERVER_CODE + "_" + code, code, (int) subject.getSession().getTimeout() / 1000);
        }
        // 回跳登录前地址
        String backurl ;//= request.getParameter("backurl");
        //if (StringUtils.isBlank(backurl)) {
            ...
            if("123456".equals(password)){
                return new UpmsResult(UpmsResultConstant.SUCCESS,  null==upmsSystem ? "/" : upmsSystem.getBasepath() + "/manage/user/init");
            }
    ...
    public Object code(HttpServletRequest request) {
        ...
        String code = RedisUtil.get(MCEP_UPMS_SERVER_CODE + "_" + codeParam);
    ...
    @ApiOperation(value = "更新session_id和code的时长")
    @RequestMapping(value = "/session", method = RequestMethod.POST)
    @ResponseBody
    public Object update(HttpServletRequest request) {
        Subject subject = SecurityUtils.getSubject();
        String sessionId = request.getParameter("upms_session_id");
        if(sessionId!=null&&sessionId.length()>0){
            String code = RedisUtil.get(MCEP_UPMS_SERVER_SESSION_ID + "_" + sessionId);
            RedisUtil.set(MCEP_UPMS_SERVER_SESSION_ID + "_" + sessionId, code, TIME_OUT);//更新MCEP_UPMS_SERVER_SESSION_ID
            RedisUtil.set(MCEP_UPMS_SERVER_CODE + "_" + code, code, TIME_OUT );//更新 MCEP_UPMS_SERVER_CODE
            String sessionStr = RedisUtil.get(MCEP_UPMS_SHIRO_SESSION_ID + "_" + sessionId);
            Session session = SerializableUtil.deserialize(sessionStr);
            UpmsSession upmsSession = (UpmsSession)session;
            upmsSession.setLastAccessTime(new Date());
            RedisUtil.set(MCEP_UPMS_SHIRO_SESSION_ID + "_" + sessionId, SerializableUtil.serialize(upmsSession), TIME_OUT);//更新MCEP_UPMS_SHIRO_SESSION_ID
        }
        return new UpmsResult(UpmsResultConstant.SUCCESS, "OK");
    }
