package net.mcep.upms.client.shiro.filter;
...
import net.mcep.upms.client.util.RequestParameterUtil;
...
public class UpmsAuthenticationFilter extends AuthenticationFilter {
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
        ...
        ssoServerUrl.append("/sso/index").append("?").append("appid").append("=").append(PropertiesFileUtil.getInstance("mcep-upms-client").get("mcep.upms.appID"));
        // 回跳地址
    ...
    private boolean validateClient(ServletRequest request, ServletResponse response) {
        ...
        // 判断局部会话是否登录
        String cacheClientSession = RedisUtil.get(MCEP_UPMS_CLIENT_SESSION_ID + "_" + session.getId());
        if (StringUtils.isNotBlank(cacheClientSession)) {
            // 更新code有效期
            RedisUtil.set(MCEP_UPMS_CLIENT_SESSION_ID + "_" + sessionId, cacheClientSession, timeOut);
            Jedis jedis = RedisUtil.getJedis();
            jedis.expire(MCEP_UPMS_CLIENT_SESSION_IDS + "_" + cacheClientSession, timeOut);
            jedis.close();
            // 移除url中的code参数
        ...
        if (StringUtils.isNotBlank(code)) {
            // 获取upms_session_id
            String upmsSessionId = request.getParameter("upms_session_id");
            ...
            try {
                ...
                if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                    ...
                    if (1 == result.getIntValue("code") && result.getString("data").equals(code)) {
                        // code校验正确，创建局部会话
                        RedisUtil.set(MCEP_UPMS_CLIENT_SESSION_ID + "_" + sessionId, code, timeOut);
                        // 保存code对应的局部会话sessionId，方便退出操作
                        RedisUtil.sadd(MCEP_UPMS_CLIENT_SESSION_IDS + "_" + code, sessionId, timeOut);
                        LOGGER.debug("当前code={}，对应的注册系统个数：{}个", code, RedisUtil.getJedis().scard(MCEP_UPMS_CLIENT_SESSION_IDS + "_" + code));
                        // 移除url中的token参数
                        String backUrl = RequestParameterUtil.getParameterWithOutCode(WebUtils.toHttp(request));
                        // 返回请求资源
                        try {
                            ...
                            HttpServletResponse httpServletResponse = WebUtils.toHttp(response);
                            /********************************************************************************************/
                            //将要把upms_session_id 放入到子系统的session中，后续需求通知oss中心更新upms_session_id的过期时间
                            String sessionStr = RedisUtil.get(MCEP_UPMS_SHIRO_SESSION_ID + "_" + sessionId);
                            Session s = SerializableUtil.deserialize(sessionStr);
                            UpmsSession upmsSession = (UpmsSession) s;
                            upmsSession.setUpmsSessionId(upmsSessionId);
                            RedisUtil.set(MCEP_UPMS_SHIRO_SESSION_ID + "_" + sessionId, SerializableUtil.serialize(upmsSession), (int)upmsSession.getTimeout());
                            /********************************************************************************************/
                            httpServletResponse.sendRedirect(backUrl.toString());
                            return true;
                        } catch (IOException e) {
