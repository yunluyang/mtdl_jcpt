...
public class RedisUtil {
	...
	public synchronized static Jedis getJedis() {
		...
		try {
			if (null != jedisPool) {
				//System.out.println("获取redis前，已活跃数量"+jedisPool.getNumActive());
				jedis = jedisPool.getResource();
				//System.out.println("获取redis后，已活跃数量"+jedisPool.getNumActive());
				try {
					jedis.auth(PASSWORD);
				} catch (Exception e) {
			...
			try {
				LOGGER.error("jedis active number : " +jedisPool.getNumActive());
				if(jedisPool.getNumActive()==MAX_ACTIVE){//如果当前活跃的数量等于最大接续数，重新创建连接池
					jedisPool.destroy();
					initialPool();
					return getJedis();
				}
