...
Mcep.dialog = {
    /**
     *弹出框选择的返回记录统一放在Mcep.dialog.record 中，统一从这里取
     */
    record : []
};
...
Mcep.Page.Tab = {
    ...
    createNewTab : function(url,name,parameter){
        ...
        var pid = currentUrl.replace(/\./g, '_').replace(/\//g, '_').replace(/:/g, '_').replace(/\?/g, '_').replace(/,/g, '_').replace(/=/g, '_').replace(/&/g, '_').replace("#", '_');
        parameter.pid = pid;
        for(var key in parameter){
            parms = parms + key + '=' + parameter[key]+'&';
        }
        if(parms.length>0){
            _url = _url + (url.indexOf("?")==-1?"?":"&") + parms.substring(0,parms.length-1);
        }
    ...
    closeCurrenPage : function () {
        ...
        if(urlParams["taskId"]!=null && urlParams["taskId"]!=""){//这是处理流程待办页面的特殊处理
            var pid = urlParams["pid"];
            pageId = pid.substring(0,pid.indexOf("8888"))+ "8888_manage_process_exection_"+urlParams["taskId"]+"_pid_"+pid;
...
Mcep.Math = {
    ...
    random : function (start,end){
        var interval = Mcep.Calc.sub(end,start);
        var ran = Mcep.Calc.mul(interval,Math.random()).toFixed(2);
        return Mcep.Calc.add(ran,start);
    }
}
